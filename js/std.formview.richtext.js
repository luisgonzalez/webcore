...
var nicEditorConfig = bkClass.extend({
	...
	iconsPath : '../img/std.form.richtext.gif',
...
var nicEditor = bkClass.extend({
	...
	getIcon : function(iconName,options) {
		...
		return {backgroundPosition : ((icon) ? ((icon-1)*-18) : 0)+'px 0px'};	
	},
...
var nicEditorInstance = bkClass.extend({
	...
	construct : function(e,options,nicEditor) {
		...
		//newX = parseInt(e.getStyle('width')) || ($e).getSize().x;
		//newY = parseInt(e.getStyle('height')) || ($e).getSize().y;
		//this.initialHeight = newY-8;
		//alert(newX + ", " + newY);
		var isTextarea = (e.nodeName.toLowerCase() == "textarea");
		if(isTextarea || this.options.hasPanel) {
			var ie7s = (bkLib.isMSIE && !((typeof document.body.style.maxHeight != "undefined") && document.compatMode == "CSS1Compat"))
            var s = {border : '1px solid #ccc', borderTop : 0, overflow : 'auto' };
			s[(ie7s) ? 'height' : 'maxHeight'] = (this.ne.options.maxHeight) ? this.ne.options.maxHeight+'px' : null;
			this.editorContain = new bkElement('DIV').setStyle(s).appendBefore(e);
			var editorElm = new bkElement('DIV').setStyle({margin: '4px', height : '120px', backgroundColor : 'white'}).addClass('main').appendTo(this.editorContain);

 ...
 * @version 1.0
 * 
 * Prrovides classes for SMTP authentication, Mail Message, Mail Identity and mail sending features.
 * The SMTP client can be configured in the app.settings file.
 *
 ...
 * @subpackage Mail
 */
class SmtpClient extends ObjectBase
{
    protected $server;
    protected $port;
    protected $username;
    ...
    {
        if (!is_string($value))
            throw new SystemException(SystemException::EX_INVALIDPARAMETER, 'Parameter must be a string');
        $this->server = $value;
    }
    ...
    {
        if (!is_int($value))
            throw new SystemException(SystemException::EX_INVALIDPARAMETER, 'Parameter must be an integer');
        $this->port = $value;
    }
    ...
    public function authenticateUser(&$tracking)
    {
        ...
        if ($smtpIn === false)
            throw new SystemException(SystemException::EX_MAILCONNECTION, $errstr);
        ...
        {
            fclose($smtpIn);
            throw new SystemException(SystemException::EX_MAILLOGIN, $tracking['auth_password']);
        }
    ...
    private static function checkResponse($response)
    {
        ...
        if ($decodedResponse->IsError)
            throw new SystemException(SystemException::EX_MAILCONNECTION, $decodedResponse->Code . ': ' . $decodedResponse->ServerMessage);
    }
    ...
    public function send($mailMessage)
    {
        ...
        $rcptList = new IndexedCollection();
        $rcptList->addRange($mailMessage->getRecipientList()->getArrayReference());
        $rcptList->addRange($mailMessage->getRecipientListCC()->getArrayReference());
        $rcptList->addRange($mailMessage->getRecipientListBCC()->getArrayReference());
 ...
 * @subpackage Mail
 */
class MailIdentity extends SerializableObjectBase
{
    ...
    public function setEmailAddress($value)
    {
        ...
        if (!$valid)
            throw new SystemException(SystemException::EX_INVALIDPARAMETER, 'value must be in a correct email address format');
 ...
 * @subpackage Mail
 */
class MailAttachment extends SerializableObjectBase
{
    protected $data;
 ...
 * @subpackage Mail
 */
class MailMessage extends SerializableObjectBase
{
    ...
    public function __construct($senderAddress, $senderName = '', $subject = '', $htmlContent = '', $plainTextContent = '')
    {
        ...
        $this->attachmentList   = new IndexedCollection();
        $this->recipientList    = new IndexedCollection();
        $this->recipientListCC  = new IndexedCollection();
        $this->recipientListBCC = new IndexedCollection();
    }
    
    /**
     * @return IndexedCollection<MailIdentity>
     */
    ...
    /**
     * @return IndexedCollection<MailAttachment>
     */
    ...
    public function addRecipient($emailAddress, $fullName = '')
    {
        ...
        $this->recipientList->addItem($recipient);
    }
    
    public function addRecipientCC($emailAddress, $fullName = '')
    {
        $recipient = new MailIdentity($emailAddress, $fullName);
        $this->recipientListCC->addItem($recipient);
    }
    
    public function addRecipientBCC($emailAddress, $fullName = '')
    {
        $recipient = new MailIdentity($emailAddress, $fullName);
        $this->recipientListBCC->addItem($recipient);
    }
    ...
    public function addAttachment($filepath, $contentType = 'application/octet-stream')
    {
        ...
        $this->attachmentList->addItem($attachment);
    }
    ...
    public function setSenderAddress($value)
    {
        ...
        if (!$valid)
            throw new SystemException(SystemException::EX_INVALIDPARAMETER, 'Invalid email address format \'' . $value . '\'.');
    ...
    public function getMailContent()
    {
        ...
        $header .= "X-Mailer: WebCore " . WEBCORE_VERSION_MAJOR . '.' . WEBCORE_VERSION_MINOR . '.' . WEBCORE_VERSION_REVISION . "\r\n";

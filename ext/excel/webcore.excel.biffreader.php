...
require_once 'webcore.excel.escher.php';
 ...
 * @subpackage Excel
 */
class ExcelShared_Excel5 extends ObjectBase
{
	/**
	 ...
	 *
	 * @param ExcelWorksheet $sheet The sheet
	...
	*/
	 ...
	 */
	public static function getDistanceX(ExcelWorksheet $sheet, $startColumn = 'A', $startOffsetX = 0, $endColumn = 'A', $endOffsetX = 0)
	{
		$distanceX = 0;

		// add the widths of the spanning columns
		$startColumnIndex = ExcelCell::columnIndexFromString($startColumn) - 1; // 1-based
		$endColumnIndex = ExcelCell::columnIndexFromString($endColumn) - 1; // 1-based
		for ($i = $startColumnIndex; $i <= $endColumnIndex; ++$i) {
			$distanceX += self::sizeCol($sheet, ExcelCell::stringFromColumnIndex($i));
		}
	 ...
	 */
	public static function getDistanceY(ExcelWorksheet $sheet, $startRow = 1, $startOffsetY = 0, $endRow = 1, $endOffsetY = 0)
	{
		$distanceY = 0;
	...
	{
		list($column, $row) = ExcelCell::coordinateFromString($coordinates);
		$col_start = ExcelCell::columnIndexFromString($column) - 1;
		$row_start = $row - 1;
		...
		if ($x1 >= self::sizeCol($sheet, ExcelCell::stringFromColumnIndex($col_start))) {
			$x1 = 0;
		}
		...
		while ($width >= self::sizeCol($sheet, ExcelCell::stringFromColumnIndex($col_end))) {
			$width -= self::sizeCol($sheet, ExcelCell::stringFromColumnIndex($col_end));
			++$col_end;
		}
		...
		if (self::sizeCol($sheet, ExcelCell::stringFromColumnIndex($col_start)) == 0) {
			return;
		}
		if (self::sizeCol($sheet, ExcelCell::stringFromColumnIndex($col_end))   == 0) {
			return;
		}
		if (self::sizeRow($sheet, $row_start + 1) == 0)  return;
		
		if (self::sizeRow($sheet, $row_end + 1) == 0) return;

		// Convert the pixel values to the percentage value expected by Excel
		$x1 = $x1	 / self::sizeCol($sheet, ExcelCell::stringFromColumnIndex($col_start))   * 1024;
		$y1 = $y1	 / self::sizeRow($sheet, $row_start + 1)   *  256;
		$x2 = $width  / self::sizeCol($sheet, ExcelCell::stringFromColumnIndex($col_end))	 * 1024; // Distance to right side of object
		$y2 = $height / self::sizeRow($sheet, $row_end + 1)	 *  256; // Distance to bottom of object

		$startCoordinates = ExcelCell::stringFromColumnIndex($col_start) . ($row_start + 1);
		$endCoordinates = ExcelCell::stringFromColumnIndex($col_end) . ($row_end + 1);

		$twoAnchor = array('startCoordinates' => $startCoordinates,
							'startOffsetX' => $x1,
							'startOffsetY' => $y1,
 ...
 * @subpackage Excel
 */
class BiffReader extends ExcelReaderBase
{
	// ParseXL definitions
	...
	/**
	 * Create a new ExcelReader_Excel5 instance
	 */
	...
	public function load($pFilename)
	{
		...
		// Initialisations
		$this->_phpExcel = new ExcelWorkbook();
		$this->_phpExcel->removeSheetByIndex(0);

		// Use ParseXL for the hard work.
		$this->_ole = new ExcelShared_OLERead();
		...
		if (!$this->_readDataOnly && $this->_drawingGroupData) {
			...
			$reader = new EscherExcel5($escherWorkbook);
		...
		foreach ($this->_sheets as $sheet)
		{
			...
			if (!$this->_readDataOnly && $this->_drawingData) {
				...
				$reader = new EscherExcel5($escherWorksheet);
			...
			foreach ($this->_objs as $n => $obj) {
				...
				list($startColumn, $startRow) = ExcelCell::coordinateFromString($spContainer->getStartCoordinates());
				list($endColumn, $endRow) = ExcelCell::coordinateFromString($spContainer->getEndCoordinates());
				...
				$width = ExcelShared_Excel5::getDistanceX($this->_phpSheet, $startColumn, $startOffsetX, $endColumn, $endOffsetX);
				$height = ExcelShared_Excel5::getDistanceY($this->_phpSheet, $startRow, $startOffsetY, $endRow, $endOffsetY);

				// calculate offsetX and offsetY of the shape
				$offsetX = $startOffsetX * ExcelShared_Excel5::sizeCol($this->_phpSheet, $startColumn) / 1024;
				$offsetY = $startOffsetY * ExcelShared_Excel5::sizeRow($this->_phpSheet, $startRow) / 256;
				...
				case 0x08:
					...
					$drawing = new ExcelWorksheet_MemoryDrawing();
					...
					case EscherBstoreContainerBSE::BLIPTYPE_JPEG:
						$drawing->setRenderingFunction(ExcelWorksheet_MemoryDrawing::RENDERING_JPEG);
						$drawing->setMimeType(ExcelWorksheet_MemoryDrawing::MIMETYPE_JPEG);
						break;

					case EscherBstoreContainerBSE::BLIPTYPE_PNG:
						$drawing->setRenderingFunction(ExcelWorksheet_MemoryDrawing::RENDERING_PNG);
						$drawing->setMimeType(ExcelWorksheet_MemoryDrawing::MIMETYPE_PNG);
						break;
					}
			...
			if ($definedName['isBuiltInName']) {
						...
						if (count($explodes) == 2) {
							if ($docSheet = $this->_phpExcel->getSheetByName($explodes[0])) {
								...
								if (count($coordinateStrings) == 2) {
									list($firstColumn, $firstRow) = ExcelCell::coordinateFromString($coordinateStrings[0]);
									list($lastColumn, $lastRow) = ExcelCell::coordinateFromString($coordinateStrings[1]);
				...
				if (count($explodes) == 2)
				{
					if ($docSheet = $this->_phpExcel->getSheetByName($explodes[0])) {
						...
						$this->_phpExcel->addNamedRange( new ExcelNamedRange((string)$definedName['name'], $docSheet, $extractedRange, true) );
					}
				}
			}
		}
	...
	private function _readFont()
	{
		...
		if (!$this->_readDataOnly) {
			...
			switch ($underlineType) {
				...
				case 0x01: $font['underline'] = ExcelStyleFont::UNDERLINE_SINGLE; break;
				case 0x02: $font['underline'] = ExcelStyleFont::UNDERLINE_DOUBLE; break;
				case 0x21: $font['underline'] = ExcelStyleFont::UNDERLINE_SINGLEACCOUNTING; break;
				case 0x22: $font['underline'] = ExcelStyleFont::UNDERLINE_DOUBLEACCOUNTING; break;
			}
	...
	private function _readXf()
	{
		...
		if (!$this->_readDataOnly) {
			...
			} elseif ($code = ExcelStyleNumberFormat::builtInFormatCode($numberFormatIndex)) {
			...
			$style['protection']['locked'] = $isLocked ?
				ExcelStyleProtection::PROTECTION_INHERIT : ExcelStyleProtection::PROTECTION_UNPROTECTED;
			// bit 1; mask 0x02; 1 = Formula is hidden
			$isHidden = (0x02 & $xfTypeProt) >> 1;
			$style['protection']['hidden'] = $isHidden ?
				ExcelStyleProtection::PROTECTION_PROTECTED : ExcelStyleProtection::PROTECTION_UNPROTECTED;
			// bit 2;
			...
			switch ($horAlign) {
				case 0: $style['alignment']['horizontal'] = ExcelStyleAlignment::HORIZONTAL_GENERAL; break;
				case 1: $style['alignment']['horizontal'] = ExcelStyleAlignment::HORIZONTAL_LEFT; break;
				case 2: $style['alignment']['horizontal'] = ExcelStyleAlignment::HORIZONTAL_CENTER; break;
				case 3: $style['alignment']['horizontal'] = ExcelStyleAlignment::HORIZONTAL_RIGHT; break;
				case 5: $style['alignment']['horizontal'] = ExcelStyleAlignment::HORIZONTAL_JUSTIFY; break;
			}
			...
			switch ($vertAlign) {
				case 0: $style['alignment']['vertical'] = ExcelStyleAlignment::VERTICAL_TOP; break;
				case 1: $style['alignment']['vertical'] = ExcelStyleAlignment::VERTICAL_CENTER; break;
				case 2: $style['alignment']['vertical'] = ExcelStyleAlignment::VERTICAL_BOTTOM; break;
				case 3: $style['alignment']['vertical'] = ExcelStyleAlignment::VERTICAL_JUSTIFY; break;
			}
	...
	private function _readSheet()
	{
		...
		$this->_sheets[] = array('name' => $rec_name, 'offset' => $rec_offset);
	}
	...
	private function _readHorizontalPageBreaks()
	{
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			for ($i = 0; $i < $nm; ++$i) {
				...
				$this->_phpSheet->setBreakByColumnAndRow($cf, $r, ExcelWorksheet::BREAK_ROW);
			}
		}
	}
	...
	private function _readVerticalPageBreaks()
	{
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			for ($i = 0; $i < $nm; ++$i) {
				...
				$this->_phpSheet->setBreakByColumnAndRow($c, $rf, ExcelWorksheet::BREAK_COLUMN);
			}
		}
	}
	...
	private function _readPageSetup()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if (!$isNotInit) {
				...
				switch ($isPortrait) {
				case 0: $this->_phpSheet->getPageSetup()->setOrientation(ExcelWorksheet_PageSetup::ORIENTATION_LANDSCAPE); break;
				case 1: $this->_phpSheet->getPageSetup()->setOrientation(ExcelWorksheet_PageSetup::ORIENTATION_PORTRAIT); break;
				}
	...
	private function _readRk()
	{
		...
		$columnString = ExcelCell::stringFromColumnIndex($column);
		...
		if ( !is_null($this->getReadFilter()) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			if (!$this->_readDataOnly) {
				...
				if (ExcelShared_Date::isDateTimeFormatCode($this->_xf[$xfindex]['numberformat']['code'])) {
					$numValue = (float) $numValue;
				}
			}

			// add cell
			$this->_phpSheet->setCellValueExplicit($columnString . ($row + 1), $numValue, ExcelCell_DataType::TYPE_NUMERIC);
		}
	}
	...
	private function _readLabelSst()
	{
		...
		if ( !is_null($this->getReadFilter()) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			if (($fmtRuns = $this->_sst[$index]['fmtRuns']) && !$this->_readDataOnly) {
				...
				$richText = new ExcelRichText($this->_phpSheet->getCell($columnString . ($row + 1)));
				$charPos = 0;
				...
				$this->_phpSheet->setCellValueExplicit($columnString . ($row + 1), $this->_sst[$index]['value'], ExcelCell_DataType::TYPE_STRING);
			}
	...
	private function _readMulRk()
	{
		...
		for ($i = 0; $i < $columns; ++$i) {
			$columnString = ExcelCell::stringFromColumnIndex($colFirst + $i);
	...
	private function _readFormula()
	{
		...
		if ( !is_null($this->getReadFilter()) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			if ( (ord($recordData{6}) == 0)
				...
				$dataType = ExcelCell_DataType::TYPE_STRING;
				...
				$dataType = ExcelCell_DataType::TYPE_BOOL;
				...
				$dataType = ExcelCell_DataType::TYPE_ERROR;
				...
				$dataType = ExcelCell_DataType::TYPE_NULL;
				$value = '';

			} else {

				// forumla result is a number, first 14 bytes like _NUMBER record
				$dataType = ExcelCell_DataType::TYPE_NUMERIC;
			...
			try {
				...
				$this->_phpSheet->getCell($columnString . ($row + 1))->setValueExplicit('=' . $formula, ExcelCell_DataType::TYPE_FORMULA);
	...
	private function _readBoolErr()
	{
		...
		if ( !is_null($this->getReadFilter()) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			case 0: // boolean
				...
				// add cell value
				$this->_phpSheet->getCell($columnString . ($row + 1))->setValueExplicit($value, ExcelCell_DataType::TYPE_BOOL);
				break;
			...
			case 1: // error type
				...
				// add cell value
				$this->_phpSheet->getCell($columnString . ($row + 1))->setValueExplicit($value, ExcelCell_DataType::TYPE_ERROR);
				break;
			}
	...
	private function _readMulBlank()
	{
		...
		if (!$this->_readDataOnly) {
			for ($i = 0; $i < $length / 2 - 3; ++$i) {
				$columnString = ExcelCell::stringFromColumnIndex($fc + $i);
	...
	private function _readLabel()
	{
		...
		if ( !is_null($this->getReadFilter()) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			$this->_phpSheet->setCellValueExplicit($columnString . ($row + 1), $value, ExcelCell_DataType::TYPE_STRING);
	...
	private function _readBlank()
	{
		...
		$columnString = ExcelCell::stringFromColumnIndex($col);
	...
	private function _readPane()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if ($this->_frozen) {
				// frozen panes
				$this->_phpSheet->freezePane(ExcelCell::stringFromColumnIndex($px) . ($py + 1));
			} else {
	...
	private function _readHyperLink()
	{
		...
		if (!$this->_readDataOnly) {
			...
			foreach (ExcelCell::extractAllCellReferencesInRange($cellRange) as $coordinate) {
	...
	private function _readImData()
	{
		...
		case 0x09: // Windows bitmap format
			...
			$drawing = new ExcelWorksheet_Drawing();
	...
	private function _readBIFF8CellAddress($cellAddressStructure)
	{
		...
		// offset: 2; size: 2; index to column or column offset + relative flags
			...
			$column = ExcelCell::stringFromColumnIndex(0x00FF & $this->_GetInt2d($cellAddressStructure, 2));
	...
	private function _readBIFF8CellRangeAddressFixed($subData)
	{
		...
		$fc = ExcelCell::stringFromColumnIndex($this->_GetInt2d($subData, 4));

		// offset: 6; size: 2; index to last column
		$lc = ExcelCell::stringFromColumnIndex($this->_GetInt2d($subData, 6));
	...
	private function _readBIFF8CellRangeAddress($subData)
	{
		...
		// bit: 7-0; mask 0x00FF; column index
		$fc = ExcelCell::stringFromColumnIndex(0x00FF & $this->_GetInt2d($subData, 4));
		...
		// bit: 7-0; mask 0x00FF; column index
		$lc = ExcelCell::stringFromColumnIndex(0x00FF & $this->_GetInt2d($subData, 6));
	...
	{
		$r = ord($rgb{0}); // offset: 0; size 1; Red component
		$g = ord($rgb{1}); // offset: 1; size: 1; Green component
		$b = ord($rgb{2}); // offset: 2; size: 1; Blue component
	...
	private function _encodeUTF16($string, $compressed = '')
	{
		...
		$result = StringHelper::convertEncoding($string, 'UTF-8', 'UTF-16LE');
	...
	{
		if ($this->_codepage)
			$result = StringHelper::convertEncoding($string, 'UTF-8', $this->_codepage);
	...
	{
		switch ($index) {
			case 0x00: return ExcelStyleBorder::BORDER_NONE;
			case 0x01: return ExcelStyleBorder::BORDER_THIN;
			case 0x02: return ExcelStyleBorder::BORDER_MEDIUM;
			case 0x03: return ExcelStyleBorder::BORDER_DASHED;
			case 0x04: return ExcelStyleBorder::BORDER_DOTTED;
			case 0x05: return ExcelStyleBorder::BORDER_THICK;
			case 0x06: return ExcelStyleBorder::BORDER_DOUBLE;
			case 0x07: return ExcelStyleBorder::BORDER_HAIR;
			case 0x08: return ExcelStyleBorder::BORDER_MEDIUMDASHED;
			case 0x09: return ExcelStyleBorder::BORDER_DASHDOT;
			case 0x0A: return ExcelStyleBorder::BORDER_MEDIUMDASHDOT;
			case 0x0B: return ExcelStyleBorder::BORDER_DASHDOTDOT;
			case 0x0C: return ExcelStyleBorder::BORDER_MEDIUMDASHDOTDOT;
			case 0x0D: return ExcelStyleBorder::BORDER_SLANTDASHDOT;
			default:   return ExcelStyleBorder::BORDER_NONE;
		}
	}
	...
	{
		switch ($index) {
			case 0x00: return ExcelStyleFill::FILL_NONE;
			case 0x01: return ExcelStyleFill::FILL_SOLID;
			case 0x02: return ExcelStyleFill::FILL_PATTERN_MEDIUMGRAY;
			case 0x03: return ExcelStyleFill::FILL_PATTERN_DARKGRAY;
			case 0x04: return ExcelStyleFill::FILL_PATTERN_LIGHTGRAY;
			case 0x05: return ExcelStyleFill::FILL_PATTERN_DARKHORIZONTAL;
			case 0x06: return ExcelStyleFill::FILL_PATTERN_DARKVERTICAL;
			case 0x07: return ExcelStyleFill::FILL_PATTERN_DARKDOWN;
			case 0x08: return ExcelStyleFill::FILL_PATTERN_DARKUP;
			case 0x09: return ExcelStyleFill::FILL_PATTERN_DARKGRID;
			case 0x0A: return ExcelStyleFill::FILL_PATTERN_DARKTRELLIS;
			case 0x0B: return ExcelStyleFill::FILL_PATTERN_LIGHTHORIZONTAL;
			case 0x0C: return ExcelStyleFill::FILL_PATTERN_LIGHTVERTICAL;
			case 0x0D: return ExcelStyleFill::FILL_PATTERN_LIGHTDOWN;
			case 0x0E: return ExcelStyleFill::FILL_PATTERN_LIGHTUP;
			case 0x0F: return ExcelStyleFill::FILL_PATTERN_LIGHTGRID;
			case 0x10: return ExcelStyleFill::FILL_PATTERN_LIGHTTRELLIS;
			case 0x11: return ExcelStyleFill::FILL_PATTERN_GRAY125;
			case 0x12: return ExcelStyleFill::FILL_PATTERN_GRAY0625;
			default:   return ExcelStyleFill::FILL_NONE;
		}
	}

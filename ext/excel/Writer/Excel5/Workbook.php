 ...
 *  License Information:
 *
 *    ExcelWriter_Excel5_Writer:  A library for generating Excel Spreadsheets
...
/**
 * ExcelWriter_Excel5_Font
 *
 * @package    WebCore
 * @subpackage Excel
 */
class ExcelWriter_Excel5_Font extends ObjectBase
{
    /**
     * BIFF version
     *
    ...
    public function writeFont()
    {
        ...
        elseif ($this->_BIFFVersion == 0x0600)
        {
            ...
            $data .= ExcelShared_String::UTF8toBIFF8UnicodeShort($this->_font->getName());
        }
    ...
    private function _mapUnderline($underline)
    {
        switch ($underline)
        {
            case ExcelStyleFont::UNDERLINE_NONE:
                return 0x00;
            case ExcelStyleFont::UNDERLINE_SINGLE:
                return 0x01;
            case ExcelStyleFont::UNDERLINE_DOUBLE:
                return 0x02;
            case ExcelStyleFont::UNDERLINE_SINGLEACCOUNTING:
                return 0x21;
            case ExcelStyleFont::UNDERLINE_DOUBLEACCOUNTING:
                return 0x22;
            default:
                return 0x00;
        }
 ...
 * @subpackage Excel
 */
class ExcelWriter_Excel5_Workbook extends ExcelWriter_Excel5_BIFFwriter
{
    /**
	 * Formula parser
	 *
	 ...
	 * Class constructor
	 *
	 * @param PHPExcel $workbook The Workbook
	 ...
	 */
	public function __construct($workbook = null, $BIFF_version = 0x0600, &$str_total, &$str_unique, &$str_table, &$colors, $parser, $tempDir = '')
	{
		...
		// Add empty sheets
		$countSheets = count($workbook->getAllSheets());
		for ($i = 0; $i < $countSheets; ++$i) {
			$phpSheet  = $workbook->getSheet($i);
	...
	{
		$xfWriter = new ExcelWriter_Excel5_Xf($style);
		...
		if (isset($this->_addedFonts[$fontHashCode])) {
			...
			$fontWriter = new ExcelWriter_Excel5_Font($style->getFont());
	...
	function _calcSheetOffsets()
	{
		...
		foreach ($this->_phpExcel->getWorksheetIterator() as $sheet) {
			if ($this->_BIFF_version == 0x0600) {
				$offset += $boundsheet_length + strlen(ExcelShared_String::UTF8toBIFF8UnicodeShort($sheet->getTitle()));
			} else {
	...
	function _storeNames()
	{
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if ($this->_phpExcel->getSheet($i)->getPageSetup()->isPrintAreaSet()) {
				// Print area
				$printArea = ExcelCell::splitRange($this->_phpExcel->getSheet($i)->getPageSetup()->getPrintArea());
				$printArea = $printArea[0];
				$printArea[0] = ExcelCell::coordinateFromString($printArea[0]);
				$printArea[1] = ExcelCell::coordinateFromString($printArea[1]);
			
				$print_rowmin = $printArea[0][1] - 1;
				$print_rowmax = $printArea[1][1] - 1;
				$print_colmin = ExcelCell::columnIndexFromString($printArea[0][0]) - 1;
				$print_colmax = ExcelCell::columnIndexFromString($printArea[1][0]) - 1;
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if ($this->_phpExcel->getSheet($i)->getPageSetup()->isColumnsToRepeatAtLeftSet() && $this->_phpExcel->getSheet($i)->getPageSetup()->isRowsToRepeatAtTopSet()) {
				...
				$colmin = ExcelCell::columnIndexFromString($repeat[0]) - 1;
				$colmax = ExcelCell::columnIndexFromString($repeat[1]) - 1;
	...
	private function _writeDefinedNameBiff8($name, $formulaData, $sheetIndex = 0, $isBuiltIn = false)
	{
		...
		$nlen = ExcelShared_String::CountCharacters($name);

		// name with stripped length field
		$name = substr(ExcelShared_String::UTF8toBIFF8UnicodeLong($name), 2);
	 ...
	 *
	 * @param ExcelWorksheet  $sheet Worksheet name
	...
	function _storeBoundsheet($sheet, $offset)
	{
		...
		switch ($sheet->getSheetState()) {
			case ExcelWorksheet::SHEETSTATE_VISIBLE:	$ss = 0x00; break;
			case ExcelWorksheet::SHEETSTATE_HIDDEN:		$ss = 0x01; break;
			case ExcelWorksheet::SHEETSTATE_VERYHIDDEN:	$ss = 0x02; break;
		...
		if ($this->_BIFF_version == 0x0600) {
			...
			$data .= ExcelShared_String::UTF8toBIFF8UnicodeShort($sheetname);
		} else {
	...
	function _storeNumFormat($format, $ifmt)
	{
		...
		if ($this->_BIFF_version == 0x0600) {
			$numberFormatString = ExcelShared_String::UTF8toBIFF8UnicodeLong($format);
	...
	function _storeDatemode()
	{
		...
		$f1904     = (ExcelShared_Date::getExcelCalendar() == ExcelShared_Date::CALENDAR_MAC_1904) ?
	...
	private function _writeMsoDrawingGroup()
	{
		...
		if ($found) {
			...
			// dggContainer
			$dggContainer = new ExcelShared_Escher_DggContainer();
			...
			// bstoreContainer
			$bstoreContainer = new ExcelShared_Escher_DggContainer_BstoreContainer();
			$dggContainer->setBstoreContainer($bstoreContainer);

			// the BSE's (all the images)
			foreach ($this->_phpExcel->getAllsheets() as $sheet) {
				foreach ($sheet->getDrawingCollection() as $drawing) {
					if ($drawing instanceof ExcelWorksheet_Drawing) {
						...
						case 1: // GIF, not supported by BIFF8, we convert to PNG
							$blipType = ExcelShared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
						...
						case 2: // JPEG
							$blipType = ExcelShared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG;
						...
						$blip = new ExcelShared_Escher_DggContainer_BstoreContainer_BSE_Blip();
						$blip->setData($blipData);

						$BSE = new ExcelShared_Escher_DggContainer_BstoreContainer_BSE();
						$BSE->setBlipType($blipType);
						$BSE->setBlip($blip);

						$bstoreContainer->addBSE($BSE);

					} else if ($drawing instanceof ExcelWorksheet_MemoryDrawing) {

						switch ($drawing->getRenderingFunction()) {

						case ExcelWorksheet_MemoryDrawing::RENDERING_JPEG:
							$blipType = ExcelShared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG;
							$renderingFunction = 'imagejpeg';
							break;

						case ExcelWorksheet_MemoryDrawing::RENDERING_GIF:
						case ExcelWorksheet_MemoryDrawing::RENDERING_PNG:
						case ExcelWorksheet_MemoryDrawing::RENDERING_DEFAULT:
							$blipType = ExcelShared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;

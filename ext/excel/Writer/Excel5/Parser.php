 ...
 * @subpackage Excel
 */
class ExcelWriter_Excel5_Parser extends ObjectBase
{
    /**
     ...
     */
    function ExcelWriter_Excel5_Parser($byte_order, $biff_version)
    {
    ...
    function _convertString($string)
    {
        ...
        {
            return pack('C', $this->ptg['ptgStr']) . ExcelShared_String::UTF8toBIFF8UnicodeShort($string);
        }
    }
     ...
     * ExcelWriter_Excel5_Workbook class.
     *
     * @access private
     ...
     *
     * @access public
     * @see ExcelWriter_Excel5_Workbook::addWorksheet()
    ...
    function _match($token)
    {
        switch ($token)
        {
            ...
            default:
                ...
                elseif (preg_match("/^\w+(\:\w+)?\![A-Ia-i]?[A-Za-z][0-9]+$/u", $token) and !ereg("[0-9]", $this->_lookahead) and ($this->_lookahead != ':') and ($this->_lookahead != '.'))
                {
                    return $token;
                }
                // If it's an external reference ('Sheet1'!A1 or 'Sheet1:Sheet2'!A1)
                ...
                elseif (preg_match("/^\w+(\:\w+)?\!([A-Ia-i]?[A-Za-z])?[0-9]+:([A-Ia-i]?[A-Za-z])?[0-9]+$/u", $token) and !ereg("[0-9]", $this->_lookahead))
                {
                    return $token;
                }
                // If it's an external range like 'Sheet1'!A1 or 'Sheet1:Sheet2'!A1:B2

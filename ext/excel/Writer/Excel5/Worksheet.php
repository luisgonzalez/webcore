 ...
 * @subpackage Excel
 */
class ExcelWriter_Excel5_Worksheet extends ExcelWriter_Excel5_BIFFwriter
{
	/**
	 * Formula parser
	 *
												...
												$parser, $tempDir = '', $phpSheet)
	{
		...
		foreach ($this->_phpSheet->getCellCollection(false) as $cell) {
			$row = $cell->getRow() - 1;
			$column = ExcelCell::columnIndexFromString($cell->getColumn()) - 1;
	 ...
	 *
	 * @access public
	 * @see ExcelWriter_Excel5_Workbook::storeWorkbook()
	 */
	function close()
	{
		...
		for ($i = 0; $i < 256; ++$i) {
			...
			if ($this->_phpSheet->getDefaultColumnDimension()->getWidth() >= 0) {
				...
				$width = ExcelSharedFont::getDefaultColumnWidthByFont($this->_phpSheet->getParent()->getDefaultStyle()->getFont());
			}

			$columnLetter = ExcelCell::stringFromColumnIndex($i);
		...
		foreach ($this->_phpSheet->getCellCollection() as $cell) {
			...
			if ($cell->getValue() instanceof ExcelRichText) {
				...
				case ExcelCell_DataType::TYPE_STRING:
		...
		if ($this->_BIFF_version == 0x0600) {
			foreach ($this->_phpSheet->getHyperLinkCollection() as $coordinate => $hyperlink) {
				list($column, $row) = ExcelCell::coordinateFromString($coordinate);
				...
				$this->_writeUrl($row - 1, ExcelCell::columnIndexFromString($column) - 1, $url);
			}
		}
	...
	private function _writeBIFF8CellRangeAddressFixed($range = 'A1')
	{
		...
		$firstCellCoordinates = ExcelCell::coordinateFromString($firstCell); // e.g. array(0, 1)
		$lastCellCoordinates  = ExcelCell::coordinateFromString($lastCell);  // e.g. array(1, 6)

		$data = pack('vvvv',
			$firstCellCoordinates[1] - 1,
			$lastCellCoordinates[1] - 1,
			ExcelCell::columnIndexFromString($firstCellCoordinates[0]) - 1,
			ExcelCell::columnIndexFromString($lastCellCoordinates[0]) - 1
		);
	...
	private function _writeNumber($row, $col, $num, $xfIndex)
	{
		...
		if (ExcelWriter_Excel5_BIFFwriter::getByteOrder()) { // if it's Big Endian
	...
	private function _writeLabelSst($row, $col, $str, $xfIndex)
	{
		...
		$str = ExcelShared_String::UTF8toBIFF8UnicodeLong($str);
	...
	function _writeUrlInternal($row1, $col1, $row2, $col2, $url)
	{
		...
		// character count
		$url_len = ExcelShared_String::CountCharacters($url);
		$url_len = pack('V', $url_len);

		$url = ExcelShared_String::ConvertEncoding($url, 'UTF-16LE', 'UTF-8');
	...
	function _storeMergedCells()
	{
		...
		foreach ($mergeCells as $mergeCell) {
			...
			$range = ExcelCell::splitRange($mergeCell);
			list($first, $last) = $range[0];
			list($firstColumn, $firstRow) = ExcelCell::coordinateFromString($first);
			list($lastColumn, $lastRow) = ExcelCell::coordinateFromString($last);

			$recordData .= pack('vvvv', $firstRow - 1, $lastRow - 1, ExcelCell::columnIndexFromString($firstColumn) - 1, ExcelCell::columnIndexFromString($lastColumn) - 1);
	...
	private function _storeRangeProtection()
	{
		foreach ($this->_phpSheet->getProtectedCells() as $range => $password) {
			...
			$recordData .= ExcelShared_String::UTF8toBIFF8UnicodeLong('p' . md5($recordData));
	...
	function _storePanes()
	{
		...
		if ($freezePane = $this->_phpSheet->getFreezePane()) {
			list($column, $row) = ExcelCell::coordinateFromString($freezePane);
			$panes[0] = $row - 1;
			$panes[1] = ExcelCell::columnIndexFromString($column) - 1;
		} else {
			// thaw panes
			return;
		}
	...
	function _storeSetup()
	{
		...
		// Page orientation
		$fLandscape = ($this->_phpSheet->getPageSetup()->getOrientation() == ExcelWorksheet_PageSetup::ORIENTATION_LANDSCAPE) ?
			0x0 : 0x1;
	...
	function _storeHeader()
	{
		...
		*/
		...
		if ($this->_BIFF_version == 0x0600) {
			$recordData = ExcelShared_String::UTF8toBIFF8UnicodeLong($this->_phpSheet->getHeaderFooter()->getOddHeader());
			$length = strlen($recordData);
		} else {
	...
	function _storeFooter()
	{
		...
		*/
		...
		if ($this->_BIFF_version == 0x0600) {
			$recordData = ExcelShared_String::UTF8toBIFF8UnicodeLong($this->_phpSheet->getHeaderFooter()->getOddFooter());
			$length = strlen($recordData);
		} else {
	...
	private function _storeBreaks()
	{
		...
		foreach ($this->_phpSheet->getBreaks() as $cell => $breakType) {
			// Fetch coordinates
			$coordinates = ExcelCell::coordinateFromString($cell);

			// Decide what to do by the type of break
			switch ($breakType) {
				case ExcelWorksheet::BREAK_COLUMN:
					// Add to list of vertical breaks
					$vbreaks[] = ExcelCell::columnIndexFromString($coordinates[0]) - 1;
					break;

				case ExcelWorksheet::BREAK_ROW:
					// Add to list of horizontal breaks
					$hbreaks[] = $coordinates[1];
					break;

				case ExcelWorksheet::BREAK_NONE:
				default:
					// Nothing to do
					break;
			}
	...
	function _positionImage($col_start, $row_start, $x1, $y1, $width, $height)
	{
		...
		if ($x1 >= ExcelShared_Excel5::sizeCol($this->_phpSheet, ExcelCell::stringFromColumnIndex($col_start))) {
			$x1 = 0;
		}
		if ($y1 >= ExcelShared_Excel5::sizeRow($this->_phpSheet, $row_start + 1)) {
			$y1 = 0;
		}
		...
		while ($width >= ExcelShared_Excel5::sizeCol($this->_phpSheet, ExcelCell::stringFromColumnIndex($col_end))) {
			$width -= ExcelShared_Excel5::sizeCol($this->_phpSheet, ExcelCell::stringFromColumnIndex($col_end));
			++$col_end;
		}

		// Subtract the underlying cell heights to find the end cell of the image
		while ($height >= ExcelShared_Excel5::sizeRow($this->_phpSheet, $row_end + 1)) {
			$height -= ExcelShared_Excel5::sizeRow($this->_phpSheet, $row_end + 1);
			++$row_end;
		}
		...
		//
		if (ExcelShared_Excel5::sizeCol($this->_phpSheet, ExcelCell::stringFromColumnIndex($col_start)) == 0) {
			return;
		}
		if (ExcelShared_Excel5::sizeCol($this->_phpSheet, ExcelCell::stringFromColumnIndex($col_end))   == 0) {
			return;
		}
		if (ExcelShared_Excel5::sizeRow($this->_phpSheet, $row_start + 1) == 0) {
			return;
		}
		if (ExcelShared_Excel5::sizeRow($this->_phpSheet, $row_end + 1)   == 0) {
			return;
		}

		// Convert the pixel values to the percentage value expected by Excel
		$x1 = $x1	 / ExcelShared_Excel5::sizeCol($this->_phpSheet, ExcelCell::stringFromColumnIndex($col_start))   * 1024;
		$y1 = $y1	 / ExcelShared_Excel5::sizeRow($this->_phpSheet, $row_start + 1)   *  256;
		$x2 = $width  / ExcelShared_Excel5::sizeCol($this->_phpSheet, ExcelCell::stringFromColumnIndex($col_end))	 * 1024; // Distance to right side of object
		$y2 = $height / ExcelShared_Excel5::sizeRow($this->_phpSheet, $row_end + 1)	 *  256; // Distance to bottom of object
	...
	private function _storeMsoDrawing()
	{
		...
		$escher = new ExcelShared_Escher();

		// dgContainer
		$dgContainer = new ExcelShared_Escher_DgContainer();
		...
		// spgrContainer
		$spgrContainer = new ExcelShared_Escher_DgContainer_SpgrContainer();
		$dgContainer->setSpgrContainer($spgrContainer);

		// add one shape which is the group shape
		$spContainer = new ExcelShared_Escher_DgContainer_SpgrContainer_SpContainer();
		...
		foreach ($this->_phpSheet->getParent()->getAllsheets() as $sheet) {
			foreach ($sheet->getDrawingCollection() as $drawing) {
				...
				if ($sheet === $this->_phpSheet) {
					...
					$twoAnchor = ExcelShared_Excel5::oneAnchor2twoAnchor($this->_phpSheet, $coordinates, $offsetX, $offsetY, $width, $height);
		...
		$writer = new ExcelWriter_Excel5_Escher($escher);
		$data = $writer->close();
